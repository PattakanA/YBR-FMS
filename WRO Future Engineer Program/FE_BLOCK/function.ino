...
void ultra_servo(int degree, char mode_steer) {
  ...
  servo2.write(mapf(max(min(middle_degree + degree, 225), 45), 0, 270, 0, 180));
}

void steering_servo(int degree) {
  servo1.write((90 + max(min(degree, 30), -30)) / 2);
}
...
void motor_and_steer(int degree) {
  ...
  // motor(35);
  motor(map(abs(degree), 0, 45, 45, 45));
}
...
void line_detection() {
  ...
  if (TURN == 'U') {
    ...
    if (blue_value < 700 || red_value < 230) {
